...
exports.createUser = function(req, done) {
	var values = [ req.body.name, req.body.surname, req.body.email,
			...
			req.body.participation, req.body.invoice, req.body.invoice_data, req.body.accommodation, req.body.accommodation_from, req.body.accommodation_to ]

	db
			.get()
			.query(
					'insert into MED_USERS (NAME, SURNAME, EMAIL, PASSWORD, REGISTERDATE, UNIVERSITY, PHONE, CONGRESSROLE, SUBJECTDESCRIPTION, '
							+ 'CONTACTCOMMENTS, CONFIRMATION, PRIVILEGES, academic_title, academic_status, master, engineer, participation, INVOICE, INVOICE_DATA, ACCOMMODATION, ACCOMMODATION_FROM, ACCOMMODATION_TO) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
exports.updateUser = function(body, done) {
	var values = [ body.name, body.surname, body.email, body.password, body.university, body.phone,
	               ...
	               body.invoice, body.invoice_data, body.accommodation, body.accommodation_from, body.accommodation_to, body.id ]
	...
	db
			...
			.query(
					'UPDATE MED_USERS set name =?, surname=?, email=?, password=?, university=?, phone=?, congressrole=?, '
							...
							+ 'participation=?, invoice=?, invoice_data=?, accommodation=?, accommodation_from=?, accommodation_to=? where ID = ?',
					values, function(err, result) {
						if (err)
							return done(err)
						done(null, result)
